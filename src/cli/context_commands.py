...
class CLIContextCommands:
    ...
    def manage_context(self, args: str = ""):
        ...
        if validation.subcommand == "":
            ...
            self.io.echo(f"Project: {self.io.style(str(status['project_path']), fg='bright_white')}")
            self.io.echo(f"Explored: {self.io.style('Yes' if status['is_explored'] else 'No', fg='green' if status['is_explored'] else 'yellow')}")
            ...
            if status.get('has_git_history'):
                self.io.echo(f"Git Branch: {self.io.style(status.get('git_branch', 'unknown'), fg='cyan')}")
                self.io.echo(f"Git Commits: {status.get('git_commits', 0)}")
            self.io.echo(f"Context Aware: {self.io.style('Enabled' if self.orchestrator.context_aware else 'Disabled', fg='green' if self.orchestrator.context_aware else 'red')}")
            ...
            self.io.echo(f"Files Cached: {self.io.style(str(mem_status['files_cached']), fg='cyan')}")
            ...
            self.io.secho("Context cache cleared.", fg="green")

        elif validation.subcommand == "clearmem":
            self.orchestrator.working_memory.clear()
            self.io.secho("Session working memory cleared.", fg="green")
            ...
            self.io.secho(f"Context awareness {status}.", fg="green" if self.orchestrator.context_aware else "yellow")

        elif validation.subcommand == "add":
            # Handle add with path argument
            if not validation.args:
                self.io.secho("Error: 'add' requires a file path argument", fg="red")
