...
class TestAgentIOIntegration:
    ...
    def test_agent_tool_request_uses_table(self):
        """Tool approval should display as formatted table."""
        from src.agent.core import CodeAgent
        from tests.helpers import ConfigurableTestOrchestrator

        io = MockRichIO(confirmations=[True, True])  # Approve action
        orch = ConfigurableTestOrchestrator(
            response_content=json.dumps({
        ...
        # Run with max 1 iteration - the read_file will be displayed
        try:
            agent.run("Read config", max_iterations=1)
        except Exception:
            pass  # May fail on missing file, that's ok

        # Should have tool request table
        tables = io.get_tables()
        tool_tables = [t for t in tables if 'Tool' in str(t.get('rows', []))]

        assert tool_tables, f"No tool request tables found. Tables: {tables}"

    def test_agent_error_uses_red_panel(self):
        """Agent errors should display in red-bordered panel."""
        from src.agent.core import CodeAgent
        from tests.helpers import ConfigurableTestOrchestrator
        ...
        # Configure orchestrator to raise error
        def raise_error(*args, **kwargs):
            raise Exception("Test error for red panel")

        orch.delegate = raise_error

        agent = CodeAgent(orchestrator=orch, io=io)

        # Run should catch error and display via IO
        try:
            agent.run("Trigger error")
        except Exception:
            pass

        # Should have error panel with red border
        panels = io.get_panels()
        error_panels = [p for p in panels if p.get('title') == 'Error']

        assert error_panels, f"No error panels found. Panels: {panels}"
        assert error_panels[0]['border_style'] == 'red'

    def test_agent_command_execution_uses_syntax(self):
        """Command execution should show in syntax block."""
        from src.agent.core import CodeAgent
        from tests.helpers import ConfigurableTestOrchestrator

        io = MockRichIO(confirmations=[True, True])
        orch = ConfigurableTestOrchestrator(
            response_content=json.dumps({
        ...
        try:
            agent.run("Check git status", max_iterations=1)
        except Exception:
            pass

        # Should have syntax block for command
        blocks = io.get_syntax_blocks()

        assert blocks, f"No syntax blocks found"
        # At least one should be shell language
        shell_blocks = [b for b in blocks if b.get('language') == 'shell']
        assert shell_blocks, f"No shell syntax blocks. Blocks: {blocks}"

    def test_agent_result_uses_green_panel(self):
        """Tool results should display in green-bordered panel."""
        from src.agent.core import CodeAgent
        from tests.helpers import ConfigurableTestOrchestrator

        io = MockRichIO(confirmations=[True])
        orch = ConfigurableTestOrchestrator(
            response_content=json.dumps({
        ...
        agent = CodeAgent(orchestrator=orch, io=io)
        # Set dry_run to bypass meaningful actions check (we're testing result display, not workflow)
        agent.dry_run = True
        agent.run("Complete task")

        # Should have result panel with green border
        panels = io.get_panels()
        result_panels = [p for p in panels
                       if p.get('border_style') == 'green'
                       or p.get('title') in ('Result', 'Final Result')]

        assert result_panels, f"No result panels found. Panels: {panels}"
