...
class TestCircuitBreakerStates:
    ...
    def test_half_open_allows_limited_calls(self):
        """Test half-open allows configured number of test calls."""
        config = CircuitBreakerConfig(
            failure_threshold=2,
            reset_timeout=0.1,
            half_open_max_calls=2,
            success_threshold=5  # Higher than max_calls to test the limit
        )
        circuit = CircuitBreaker("test", config=config)
        ...
        # Should allow up to 2 calls in half-open
        func_success = Mock(return_value="success")

        result1 = circuit.call(func_success)
        assert result1 == "success"

        result2 = circuit.call(func_success)
        assert result2 == "success"

        # Third call should be blocked (exceeded half_open_max_calls)
