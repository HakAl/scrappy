...
### 1. Add .git/ to blocked paths
```python
# src/agent_tools/components/command_security.py
BLOCKED_PATHS = [".git/", ".git\\"]  # Add to existing blocklist
```

### 2. Add disclaimer banner
```python
# src/cli/interactive_banner.py
DISCLAIMER = """
[yellow]WARNING: This tool can modify files and execute commands.
Review actions carefully before confirming.[/yellow]
"""
```
...
# Wrap provider errors with context
    ...
    raise UserFacingError(
        ...
        "Check your API key and rate limits with /limits"
    )
```

### 5. Update welcome banner
```
Welcome to Scrappy!
...
### 6. Fix SESSION.md documentation
Remove references to `/session save/load/toggle` - these no longer exist with automatic persistence.

---

## Medium Effort Items

### 1. `scrappy init` Setup Wizard
...
def init():
    ...
    click.echo("Welcome to Scrappy setup!\n")

    # Check for existing config
    if has_api_key():
        if not click.confirm("Config exists. Reconfigure?"):
            return

    # Prompt for API keys
    click.echo("Enter your API keys (leave blank to skip):\n")

    github_key = click.prompt("GitHub Token", default="", show_default=False)
    groq_key = click.prompt("Groq API Key", default="", show_default=False)
    # ... etc

    # Save to .env or config file
    save_config(...)

    click.echo("\nSetup complete! Run 'scrappy' to start.")
```
...
| Doc | Issue | Status |
|-----|-------|--------|
| SESSION.md | References removed commands | FIXED |
| CLI.md | Missing /history command | FIXED |
| README.md | Stale features/roadmap | FIXED |
| TOOLS.md | Missing SemanticSearchTool | FIXED |
...
### Pre-Release (Do Now)
- [ ] Add .git/ to blocked paths (command_security.py)
- [ ] Add disclaimer banner on startup
- [ ] Add version command and __version__
- [ ] Improve error messages (wrap provider errors with context)
- [x] ~~Fix SESSION.md documentation~~ (done)
- [x] ~~Add /history to CLI.md~~ (done)
- [x] ~~Update README~~ (done)

### Release Candidate (RC)
- [ ] Implement `scrappy init` wizard (setup flow)
- [ ] Add dependency check on startup (warn if git missing)
- [ ] Fix progress bar numeric values (AGENT_BUGS issue)
