 Unicode defines specific blocks for emoji characters:
  ...
  Legacy Symbol Blocks (some rendered as emoji):
  ...
  The Unicode Consortium maintains an official list at https://unicode.org/Public/emoji/, and each Unicode version
  adds new emoji to these blocks. This makes it straightforward to detect or filter emojis programmatically by
  checking code point ranges.
...
tests/test_no_emojis.py uses a practical approach using Unicode range checking with static analysis
  ...
  This catches attempts to add emojis like "Success! 🎉" or "Error ❌".
